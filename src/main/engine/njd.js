...
/** @returns {Array<object>} NJD nodes */
function parseTextAnalysis(trace) {
  ...
  for (const raw of block.split('\n')) {
    const line = raw.trim();
    if (!line) continue;
    // The surface form can itself contain a comma (e.g. the "," symbol), so split
    // from the right: the last three fields are acc/mora, chain_rule, chain_flag.
    const parts = line.split(',');
    if (parts.length < 13) continue;
    const chainFlag = parseInt(parts[parts.length - 1], 10);
    const chainRule = parts[parts.length - 2];
    const accMora = parts[parts.length - 3];
    const head = parts.slice(0, parts.length - 3);
    // Re-join any surplus leading fields back into `string`.
    while (head.length > 10) { head[0] = head[0] + ',' + head[1]; head.splice(1, 1); }

    const node = {};
    FIELDS.forEach((name, i) => { node[name] = head[i]; });
    const [acc, moraSize] = accMora.split('/');
    node.acc = parseInt(acc, 10) || 0;
    node.moraSize = parseInt(moraSize, 10) || 0;
    node.chainRule = chainRule;
    node.chainFlag = Number.isNaN(chainFlag) ? -1 : chainFlag;
    nodes.push(node);
  }
  return nodes;
}

/** Extract the label strings from the "[Output label]" block (reference/debug use). */
function parseOutputLabel(trace) {
...
module.exports = { parseTextAnalysis, parseOutputLabel };
